...
function Home() {
    ...
    const registerHandler = (e) => {
        ...
        writeUserData(`Users/${user.uid}`, { uuid: user.uid, rol: 'Cliente', pais, ...state }, setUserSuccess)
        router.push('/')
                ...
                <form className={`space-y-6 lg:space-y-3  rounded-[30px] w-[100%] max-w-[400px] p-5 h-auto px-5 py-10 lg:p-10 z-10 lg:scale-110`} onSubmit={registerHandler} >
                    ...
                    <div className='space-y-4 bg-[#00061860] p-5  rounded-[10px] lg:space-y-3'>
                        ...
                        <div>
                            ...
                            <Select arr={countries && countries.map((i) => i.translations.spa.official) ? [...countries.map((i) =>  <div className='flex items-center '> <img src={i.flags.png} className='mr-5 inline w-[30px] border-[1px]' alt="" />{i.translations.spa.official}</div> )] : []} name='Ciudad' click={onClickHandlerCountry} />
                        </div>
                        <div>
